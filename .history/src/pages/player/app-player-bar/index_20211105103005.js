...
export default memo(function HYAppPlayerBar() {
  ...
  useEffect(() => {
    ...
    audioRef.current
      ...
      .catch((reason) => {
        setIsPlaying(false)
        message.error("Can't play the selected song due to ")
      });
    // 如果不是首次加载: 播放音乐
  ...
  const changingVolume = useCallback(
    (value) => {
      if (value < 99 && value > 1) audioRef.current.volume = value / 100;
    }
  ,[])
      ...
      <div className="content w980">
        ...
        <Operator playSequence={sequence}>
          ...
          <div className="right">
            ...
            <Tooltip
              title={["Loop", "Random", "Single"].filter(
                (item, index) => index === sequence
              )}
            >
              <button
                href="/#"
            ...
            className="btn playlist sprite_player"

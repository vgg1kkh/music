...
export const getNewAlbumAction = (limit) => {
  return (dispatch) => {
    getNewAlbums(limit).then((res) => {
      res.albums && dispatch(changeNewAlbumAction(res.albums.slice(0, 10)));
    });
  };
};
...
export const getTopListAction = (id) => {
  return (dispatch) => {
    getTopListDetail(id).then((res) => {
      dispatch(changeTopListAction(id,res.playlist))
      // console.log(res.data.playlist) 
    })    
  };
};
const changeSettleSingerAction = (data)=>{
  return{
    type: CHNAGE_SETTLE_SINGER,
    payload:data
  }
...
export const getSettleSingerAction = (limit,area)=>{
  return dispatch => {
    getSettleSinger(limit,area).then(res=>dispatch(changeSettleSingerAction(res.artists)))
  }
}

...
export default memo(function HYAppFooter() {
  ...
  const showNavItems = (item,index) =>{
    if(index<3) {return (
        <NavLink key={index} to={item.link} className="nav-item">{item.title}</NavLink>
    )}else{
        return (
            <a key={index} href={item.link} className="nav-item" rel="noreferrer" target="_blank">{item.title}</a>
        )
    }
